class Visualizer{
    static drawNetwork(ctx,network){
        ...
        for(let i = network.levels.length-1;i>=0;i--){
            const levelTop = top + lerp(height-levelHeight,0,network.levels.length==1?0:i/(network.levels.length-1));
            ctx.setLineDash([7,3]);
    ...
    static drawLevel(ctx,level,left,top,width,height,labels){
        ...
        const {inputs,outputs,weights,biasses} = level;
        ...
        for(let i = 0;i<outputs.length;i++){
            ...
            ctx.strokeStyle = getRGBA(biasses[i]);
            ctx.setLineDash([3,3]);
            ctx.stroke();
            ctx.setLineDash([]);
